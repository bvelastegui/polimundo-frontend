<template>
  <v-layout column>
    ...
    <v-card
        ...
        :loading="loading"
    >
      <v-card-text v-if="!loading" key="subtitle">
        Resultados de la búsqueda <strong v-text="searchTerms"></strong>
      </v-card-text>
      <v-card-text v-else-if="loading" key="subtitle">Buscando...</v-card-text>
    </v-card>
    <v-row v-if="!loading && flights.length === 0" key="results" class="mt-3">
      <v-card>
        <v-card-text>No se encontraron resultados para tu búsqueda</v-card-text>
      </v-card>
    </v-row>
    <v-simple-table v-else key="results" class="mt-3" >
      <template v-slot:default>
        ...
        >
          <td>
            <v-avatar class="my-3" color="indigo">
              <v-icon dark>
                mdi-airplane
              </v-icon>
            </v-avatar>
          ...
          <td v-if="flight.segment.length > 1" key="scales">
            {{ flight.segment.length - 1 }} escalas (cambio de avión)
          </td>
          <td v-else key="scales">
            Viaje directo
            <small class="d-block">sin escalas</small>
          </td>
          <td v-if="flight.segment.length > 1" key="dest">
          ...
          <td class="text-right"><small>USD</small> {{ flight.price }}</td>
          <td class="text-right">
...
export default {
  computed: {
    ...
    time() {
      return (duration) => {
        return (duration / 60).toFixed(0) + 'h'
      }
    }
  },
  async created() {
